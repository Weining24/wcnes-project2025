...
///  4 + 2 * x - for the minimum cycles low
const SET_X: u8 = 0;
const SHORT: u8 = 4 + 2 * SET_X;
const LONG: u8 = 2 * SHORT;

fn chips_to_waves(bit_chip2: u8) -> [Level; 3] {
    match bit_chip2 {
        0b00 => {
            //0000111111110000
            [Level::Low(SHORT), Level::High(LONG), Level::Low(SHORT)]
        }
        0b01 => {
            //0000000011111111
            [Level::Low(LONG), Level::High(LONG), Level::Nop]
        }
        0b10 => {
            // 1111111100000000
            [Level::High(LONG), Level::Low(LONG), Level::Nop]
        }
        0b11 => {
            //1111000000001111
            [Level::High(SHORT), Level::Low(LONG), Level::High(SHORT)]
        }
