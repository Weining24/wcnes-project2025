...
fn setup_clocks(
    ...
    // constants used for the two post dividers
    const DIV1: u8 = 6;
    const DIV2: u8 = 2;
...
fn main() -> ! {
    ...
    // number of repetitions of waveform
    let rep = |n: u8| core::iter::repeat(n).take(4);
    // convert the bytes to PIO O-QPSK
    let iter_frame = convert(&generated_frame_bytes, rep);
    ...
    let iter_string = convert(&str_vec, rep);
